...
public class FTPClientImportType extends ServiceType {
	...
	public IoSession initClientMina(MyClientHandler myClientHandler,ServiceHandle handle,String ip,String port,boolean type) {
		...
		try {
			...
			connector.setHandler(myClientHandler);
			connector.getFilterChain().addLast("codec",new ProtocolCodecFilter(new ClientCodecFactory("GB2312","UTF-8")));// 添加Filter对象
		} catch (Exception e2) {
			e2.printStackTrace();
		...
		while (true) {
			try {
				...
				try {
					...
					System.out.println("Start the " + (count + 1) + " client connection");
